...
void InitializeJsonFile() {
	...
	if (gCharacterStructure.empty()) {
		l.Get()->error("Cannot initialize characters: gCharacterStructure is empty!");
		l.Get()->flush();
		return;
	}
...
uint8_t ReadByteFromJson(uint32_t offset) {
	...
	if (offset < 64) {
		...
		if (characterOffset < defaultTotalBytes) {
			...
			l.Get()->info("Reading default character: {}", characterId);
			l.Get()->flush();
			...
			l.Get()->info("Reading addon character: {}", characterId);
			l.Get()->flush();
